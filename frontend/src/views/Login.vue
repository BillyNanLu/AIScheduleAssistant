...
import {
  ...
  ElButton, ElLink, ElMessage, ElIcon
} from 'element-plus'
import {
  School, HomeFilled, User, Lock, Phone, Message, Calendar as MessageIcon
...
const handleLogin = async () => {
  try {
    ...
    if (res.code === 0) {
      ...
      // 登录成功后获取用户信息
      const userInfoRes = await userInfoService()
      if (userInfoRes.code === 0) {
        // 存储用户信息
        const userInfoStore = useUserInfoStore()
        userInfoStore.setInfo(userInfoRes.data)
        // 根据用户角色跳转不同页面
        const redirectPath = userInfoRes.data.role === 0 ? '/admin' : '/'
    ...
    <ElHeader class="layout-header" fixed>
      <div class="header-container">
        ...
        <div class="logo">
          ...
          <span class="logo-text">智能日程管理助手</span>
        </div>
        <!-- 导航操作：「返回主页」按钮 -->
        ...
        >
          ...
          <ElFormItem label="用户名" prop="username">
            <ElInput
                ...
                :prefix-icon="User"
                :placeholder="isRegister ? '请输入用户名（5-16位，支持字母/数字/下划线），注册后不支持修改' : '请输入用户名'"
                clearable
            />
          </ElFormItem>
          ...
          <ElFormItem label="密码" prop="password">
            <ElInput
                ...
                :prefix-icon="Lock"
                type="password"
                :placeholder="isRegister ? '请输入密码（5-16位，需包含字母和数字）' : '请输入密码'"
                clearable
            />
          </ElFormItem>
          ...
          <template v-else>
            <ElFormItem class="remember-item">
              <ElLink type="primary" :underline="false" @click="router.push('/forgot')" class="forgot-link">
                忘记密码？
              </ElLink>
            </ElFormItem>
          </template>
